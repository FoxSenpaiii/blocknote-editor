...
export const initUI = (vditor: IEditor) => {
	...
	setEditMode(vditor, vditor.options.mode, afterRender(vditor) as string);
...
export const setPadding = (vditor: IEditor) => {
	...
	if (vditor.wysiwyg?.element.parentElement?.style.display !== 'none') {
		const padding = (vditor.wysiwyg!.element.parentElement!.clientWidth - vditor.options.preview?.maxWidth!) / 2;
	...
	if (vditor.ir?.element.parentElement?.style.display !== 'none') {
		const padding = (vditor.ir!.element.parentElement!.clientWidth - vditor.options.preview?.maxWidth!) / 2;
	...
	if (vditor.preview?.element.style.display !== 'block') {
		...
		vditor.toolbar!.element!.style.paddingLeft =
			5 + (vditor.options.outline?.position === 'left' ? vditor.outline.element.offsetWidth : 0) + 'px';
	}
};

export const setTypewriterPosition = (vditor: IEditor) => {
...
const afterRender = (vditor: IEditor) => {
