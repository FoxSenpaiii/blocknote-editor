...
class Undo {
	...
	public clearStack(vditor: IEditor) {
		this.resetStack();
		this.resetIcon(vditor);
	}

	public resetIcon(vditor: IEditor) {
		if (!vditor.toolbar) {
			return;
		}
	...
	public addToUndoStack(vditor: IEditor) {
	...
	private renderDiff(state: DiffMatchPatch.patch_obj[], vditor: IEditor, isRedo: boolean = false) {
		let text;
		if (isRedo) {
