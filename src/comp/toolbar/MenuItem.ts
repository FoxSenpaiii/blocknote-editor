	...
	constructor(vditor: IEditor, menuItem: IMenuItem) {
		...
		this.element.children[0].addEventListener(getEventName(), (event) => {
			...
			if (vditor.currentMode === 'wysiwyg') {
				...
				processToolbar(vditor, this.element.children[0], menuItem.prefix || '', menuItem.suffix || '');
			} else {
				processToolbarSV(vditor, this.element.children[0], menuItem.prefix || '', menuItem.suffix || '');
			}
		});
	}
}
