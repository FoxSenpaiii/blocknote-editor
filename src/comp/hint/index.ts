...
import { IHintExtend, IEditor, IHintData } from '$type';
export class Hint {
	...
	public render(vditor: IEditor) {
		...
		if (typeof key === 'undefined') {
			...
			Object.keys(emojiHint).forEach((keyName) => {
				if (keyName.startsWith(key.toLowerCase())) {
	...
	public genHTML(data: IHintData[], key: string, vditor: IEditor) {
		if (data.length === 0) {
			this.element.style.display = 'none';
			return;
		}
...
${i === 0 ? "class='vditor-hint--current'" : ''}> ${html}</button>`;
	...
	public fillEmoji = (element: HTMLElement, vditor: IEditor) => {
		this.element.style.display = 'none';
	...
	public select(event: KeyboardEvent, vditor: IEditor) {
