...
export const previewImage = (oldImgElement: HTMLImageElement, lang: keyof II18n = 'zh_CN', theme = 'classic') => {
    ...
    <div class="vditor-img__img" onclick="this.parentElement.outerHTML = '';document.body.style.overflow = ''">
      <img style="width: ${oldImgElement.width}px;height:${oldImgElement.height}px;transform: translate3d(${
			oldImgRect.left
		}px, ${oldImgRect.top - height}px, 0)" src="${oldImgElement.getAttribute('src')}">
    </div>
</div>`
	);
	...
	const translate3d = `translate3d(${Math.max(0, window.innerWidth - oldImgElement.naturalWidth) / 2}px, ${
		Math.max(0, window.innerHeight - height - oldImgElement.naturalHeight) / 2
	}px, 0)`;
	setTimeout(() => {
	...
	btnElement?.addEventListener('click', () => {
		...
		if ((deg / 90) % 2 === 1 && oldImgElement.naturalWidth > imgElement.parentElement!.clientHeight) {
			...
			}px, ${oldImgElement.naturalWidth / 2 - oldImgElement.naturalHeight / 2}px, 0) rotateZ(${deg}deg)`;
		} else {
