...
import { getCursorPosition, getEditorRange, getSelectPosition, setRangeByWbr } from '$util/selection';
...
import { ICommentsData } from '$type/index';
...
<div class="vditor-panel vditor-panel--none">
	...
	public getComments(vditor: IEditor, getData = false) {
	...
	public triggerRemoveComment(vditor: IEditor) {
			...
			if (event.target.isEqualNode(this.element) && this.element.lastElementChild && range.collapsed) {
				...
				if (event.y > lastRect.top + lastRect.height) {
					if (
						...
						this.element.insertAdjacentHTML('beforeend', `<p data-block="0">${Constants.ZWSP}<wbr></p>`);
