...
export const input = (vditor: IEditor, range: Range, event?: InputEvent) => {
	...
	if (
		...
		allFootnoteElement!.forEach((item, index) => {
			if (index === 0) {
				firstFootnoteElement = item as HTMLElement;
			} else {
		...
		vditor
			.wysiwyg!.element.querySelectorAll(".vditor-wysiwyg__preview[data-render='2']")
			//@ts-ignore
