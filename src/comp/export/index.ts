...
export const download = (vditor: IEditor, content: string, filename: string) => {
...
export const exportMarkdown = (vditor: IEditor) => {
	const content = getMarkdown(vditor);
	download(vditor, content, content.substr(0, 10) + '.md');
};

export const exportPDF = (vditor: IEditor) => {
	//@ts-ignore
...
export const exportHTML = (vditor: IEditor) => {
