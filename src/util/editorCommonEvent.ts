...
import { processHeading } from '$comp/ir/process';
import { processKeydown as irProcessKeydown } from '$comp/ir/processKeydown';
import { getMarkdown } from '$comp/markdown/getMarkdown';
import { previewImage } from '$comp/preview/image';
import { processHeading as processHeadingSV } from '$comp/sv/process';
import { processKeydown as mdProcessKeydown } from '$comp/sv/processKeydown';
import { setEditMode } from '$comp/toolbar/EditMode';
import { hidePanel } from '$comp/toolbar/setToolbar';
import { afterRenderEvent } from '$comp/wysiwyg/afterRenderEvent';
import { processKeydown } from '$comp/wysiwyg/processKeydown';
import { removeHeading, setHeading } from '$comp/wysiwyg/setHeading';
...
import { IEditor, IMenuItem } from '$type';

export const focusEvent = (vditor: IEditor, editorElement: HTMLElement) => {
...
export const dblclickEvent = (vditor: IEditor, editorElement: HTMLElement) => {
	//@ts-ignore
...
export const blurEvent = (vditor: IEditor, editorElement: HTMLElement) => {
...
export const dropEvent = (vditor: IEditor, editorElement: HTMLElement) => {
...
export const copyEvent = (
	...
	copy: (event: ClipboardEvent, vditor: IEditor) => void
) => {
...
export const scrollCenter = (vditor: IEditor) => {
...
export const hotkeyEvent = (vditor: IEditor, editorElement: HTMLElement) => {
	//@ts-ignore
...
export const selectEvent = (vditor: IEditor, editorElement: HTMLElement) => {
	//@ts-ignore
	editorElement.addEventListener('selectstart', (event: Event & { target: HTMLElement }) => {
		editorElement.onmouseup = () => {
			setTimeout(() => {
				...
				} else if (vditor.currentMode === 'wysiwyg' && vditor.options.comment?.enable) {
					vditor.wysiwyg?.hideComment();
				}
			});
		};
