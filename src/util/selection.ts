...
export const getEditorRange = (vditor: IEditor) => {
	let range: Range;
...
export const setSelectionByPosition = (start: number, end: number, editor: HTMLElement) => {
	...
	while (!stop && pNode) {
		...
		if (!foundStart && start >= charIndex && start <= nextCharIndex) {
			...
			} else if (pNode.childNodes[0].nodeType === 3) {
...
export const setRangeByWbr = (element: HTMLElement, range: Range) => {
	...
	} else if (wbrElement.previousElementSibling.isSameNode(wbrElement.previousSibling)) {
...
export const insertHTML = (html: string, vditor: IEditor) => {
